...
A new Hermes candidate was intentionally not built. The supported Python 3.12 suite is complete at 613 collected, 613 passed, 0 failed, and 0 skipped. One real zero-tool response failed exact-schema validation and three later exact-schema requests returned HTTP 429, so the required Hermes pilots have not all passed.
...
An additional 2026-07-23 University Assignment check found that the untouched recovery candidate could create a live referenced Word assignment, but could not complete its natural-language save follow-up and produced 129 words for a requested 300-word essay. The document was recovered externally under `.test_tmp` without affecting another Word document. External recovery is not a packaged pass. This adds save routing and assignment-length compliance to the candidate release blockers; it does not change the source suite result or authorize a rebuild before the Hermes gates pass.
