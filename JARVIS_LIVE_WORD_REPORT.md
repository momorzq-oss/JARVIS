...
A fresh short cited solar-energy report completed in live Word mode. `Close Word, please.` during the save confirmation remained in the contextual pending dialogue and did not dispatch `app.close`. `Yes.` saved and verified `.test_tmp\pending close validation.docx` at 17,661 bytes. A subsequent Close Word completed in one second, emptied the JARVIS registry, and preserved the pre-existing user Word and Chrome processes. The complete suite reports 624 collected, 624 passed, 0 failed, and 0 skipped.

On 2026-07-23, a fresh blank Word document exposed that `Save this document to <path>` fell to chat even though `Save it to <path>` worked. The shared save matcher accepted `this` but not `this document`. It now accepts `it`, `this`, `this document`, `the document`, and equivalent file wording through one deterministic route. Live patched validation inserted text, saved `.test_tmp\source_word_context_save_20260723.docx`, reopened it with `python-docx`, verified the exact content, and closed only the JARVIS-owned document while the existing Word process stayed responsive.

The same loop found three related shared-state defects. `University` inside a proposal topic no longer activates Assignment Mode, `brief` as an adjective no longer turns a proposal into a research brief, and a slow Office action now binds its initiating command before pause/resume can replace ambient command text. Live pause held Word content at exactly 28 characters, resume completed and logged the original proposal request, and cancel closed the exact unsaved test document while keeping the task `CANCELLED`. Emergency stop repeated that cleanup and produced no post-stop Piper synthesis or playback.
